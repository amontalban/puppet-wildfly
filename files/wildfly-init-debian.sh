 ...
 start)
	...
	if [ $status_start -eq 3 ]; then
		...
		if [ "$JBOSS_MODE" = "standalone" ]; then
			...
			--exec "$JBOSS_SCRIPT" -- -c $JBOSS_CONFIG -b=$JBOSS_BIND_ADDRESS -bmanagement=$JBOSS_BIND_ADDRESS_MANAGEMENT >> $JBOSS_CONSOLE_LOG 2>&1 &
		else
			start-stop-daemon --start --user "$JBOSS_USER" \
			--chuid "$JBOSS_USER" --chdir "$JBOSS_HOME" --pidfile "$JBOSS_PIDFILE" \
			--exec "$JBOSS_SCRIPT" -- --domain-config=$JBOSS_DOMAIN_CONFIG \
			--host-config=$JBOSS_HOST_CONFIG -b=$JBOSS_BIND_ADDRESS -bmanagement=$JBOSS_BIND_ADDRESS_MANAGEMENT >> $JBOSS_CONSOLE_LOG 2>&1 &
		fi
